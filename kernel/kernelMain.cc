...
// get tid and convert this into pointer
    ...
    pTask_Rcv = pTask->pTaskRecv   = aTDTable.getPtTask(rcvTid);
...
// get the pointer of msg queue of each task
	...
	// remove receiver task from original priority queue
	    Error2(ErrorNone, "sender task[%d] and prio[%d]",
		    pTask->getTid(), pTask->getPriority());
	    Error3(ErrorNone, "receiver task[%d] and prio[%d] state %d",
		    pTask_Rcv->getTid(), pTask_Rcv->getPriority(),
		    pTask_Rcv->getStateNum());
	    ...
	    // XXX XXX Because some task is blocked as Event
...
//  for return value , real copy data size
    ...
    pTask_Rcv = pTask->pTaskRecv   = aTDTable.getPtTask(getParameter(1));
...
// XXX for priority inheritance 
#else

// if priority is inherited
